...
const errorMessageHtml = computed(() => {
  ...
  return `<h3 class="card-title error-title">❌ Puzzle Generation Failed</h3>
<p class="card-subtitle">${props.error.message}</p>
...
<template>
  <div class="generation-card">
    ...
    <div v-if="genStatusMessage && gameStatus === GameStatuses.GENERATING" class="generation-content">
      ...
      <p class="card-note">
        The generation process will continue until either the target is achieved or the time/iteration limit is reached. If the process stops due to these limits, the closest possible solution will be provided. During this process, the algorithm also performs optimization by making small adjustments to minimize the number of solutions on the board.
      </p>
    </div>
    ...
    <div v-if="gameStatus === GameStatuses.GENERATING_ERROR" class="generation-content error">
      <div v-html="errorMessageHtml"></div>
    </div>
  </div>
</template>
